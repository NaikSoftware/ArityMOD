...
import ua.naiksoftware.aritymod.core.util.Util;
...
abstract class GLView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    protected void glDraw() {
        if (hasSurface && !paused) {
            ...
            if (!egl.eglSwapBuffers(display, surface)) {
                Timber.e("swapBuffers error %s", egl.eglGetError());
            }
            if (egl.eglGetError() == EGL11.EGL_CONTEXT_LOST) {
                Timber.e("EGL context lost %s", this);
                paused = true;
            }
            if (mIsLooping) {
                requestDraw();
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        Timber.d("surfaceCreated %s", this);
    }

    public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
        Timber.d("surfaceChanged %d %s", format, this);
        this.width = width;
        this.height = height;
    ...
    public void surfaceDestroyed(SurfaceHolder holder) {
        Timber.d("surfaceDestroyed %s", this);
        hasSurface = false;
        deinitGL();
    }
